...
describe('Request listener', () => {
    ...
    const mockRes = { end: jest.fn(), writeHead: jest.fn() } as unknown as ServerResponse
    ...
    it('Should serve image when correct url is passed', async () => {
        ...
        expect(serveImageSpy).toHaveBeenCalledWith(mockRes, {imageName, searchParams: new URLSearchParams()})
    });

    it('Should return Not Found when incorrect url is passed', async () => {
        await reqListener({...mockReq, url: '/invalid'} as unknown as IncomingMessage, mockRes);
    ...
    it('Should return Internal Server Error when image serve fails',async () => {
        ...
        await reqListener(mockReq, mockRes);
        expect(mockRes.end).toHaveBeenCalledWith(safeStringify({response: `An error occured ${errMessage}`}))
    })
})