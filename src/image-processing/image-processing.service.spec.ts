...
describe("Image processing service", () => {
  ...
  describe("When called with resolution as a search param", () => {
    ...
    it("Should resize image and pipe it to response", async () => {
      ...
      await imageProcessingService.serveImage(mockRes, {
        imageName,
        searchParams,
      });
  ...
  describe("When called without resolution as a search param", () => {
    ...
    `(
      "Should handle emitted error based on error message $mockErrMessage and return $statusCode",
      async ({ mockErrMessage, statusCode }) => {
        const mockErr = {
        ...
        createReadStreamSpy.mockReturnValue(mockReadStream);
        pipelineSpy.mockReturnValue({ catch: jest.fn() });
        await imageProcessingService.serveImage(
          { ...mockRes, headersSent: false } as unknown as ServerResponse,
          { imageName, searchParams: new URLSearchParams("") },
        );
        mockReadStream.emit("error", mockErr);
        expect(mockRes.end).toHaveBeenCalledWith(
          safeStringify({ response: `An error occured: ${mockErr.message}` }),
        );
