...
import {
  cachedImagesDirectoryPath,
  imagesDirectoryPath,
} from "../common/constants/image-processing.constants";
...
class ImageProcessing {
  async serveImage(
    ...
    try {
      const imageStream = imageProcessingService.getImageStream(
        imageName,
        imagesDirectoryPath,
      );
      ...
      if (searchParams.has("resolution")) {
        await this.pipeResizedImage(imageStream, res, {
          imageName,
          searchParams,
        });
      } else {
  ...
  private getImageStream(imageName: string, dirPath: string): fs.ReadStream {
    try {
  ...
  private async isCachedImage(
    imageName: string,
    { width, height }: { width: number; height: number },
  ): Promise<boolean> {
    try {
      await access(
        `${__dirname}${cachedImagesDirectoryPath}${parse(imageName).name}_${width}_${height}.jpg`,
        fs.constants.F_OK,
      );
      return true;
    } catch (error) {
    ...
    try {
      ...
      if (await this.isCachedImage(imageName, { width, height })) {
        ...
        const cachedReadStream = this.getImageStream(
          `${parse(imageName).name}_${width}_${height}.jpg`,
          cachedImagesDirectoryPath,
        );
        ...
        await pipeline(readStream, transformer, res).catch(
          this.logPipelineErrors,
        );
        // saving image in memory
        transformer.toFile(
          `images/cached/${parse(imageName).name}_${width}_${height}.jpg`,
        );
      }
    } catch (error) {
  ...
  private handleEmittedErrors(stream: Stream, res: ServerResponse): void {
    stream.on("error", (error) => {
      if (!res.headersSent) {
        const statusCode = error.message.includes(NO_FILE_OR_DIR)
